...
const isScriptsAuthorized = async (request, env, url) => {
  ...
  try {
    ...
    const expected = JSON.parse(results[0].value);
...
def health_check_loop():
    ...
    while True:
        ...
        # 1. 应用层：多维 HTTP 探针（L5: 改用原生 socket 建连 + 超时，避免每轮 fork 多个 curl 子进程）
        endpoints = [
        ...
        for probe_host, probe_port in endpoints:
            try:
                ...
                s.settimeout(5)
                s.setsockopt(socket.SOL_SOCKET, 25, target_tun.encode('utf-8'))
...
def get_best_candidate():
    ...
    # L6: 锁内只做最小必要的快照与写操作，排序/筛选移到锁外，避免长持锁阻塞 harvest/blacklist
    with reservoir_lock:
        ...
        if candidates:
            # 锁外返回前 pop 候选：但 candidates 是快照引用，回到锁内移除更安全
...
`;
    ...
    if (url.pathname === "/api/proxies") {
      ...
      if (results) {
        for (let server of results) {
          ...
          proxyList.push(`socks5://${PROXY_USER}:${PROXY_PASS}@${server.ip}:${port}#${activeNode.country}_ActiveNode_${activeNode.node_ip || 'IP'}`);
        }
      }
    ...
    if (url.pathname === "/api/nodes") {
      try {
        // M2: 短缓存（4s）遏制多端轮询对 D1 的读放大（原每端 5s 一次裸读）
        const cacheKey = new Request("https://cache.internal/api/nodes");
        ...
        const resp = new Response(JSON.stringify(results || []), {
          headers: { "Content-Type": "application/json", "Cache-Control": "public, max-age=4" }
        });
        ctx.waitUntil(caches.default.put(cacheKey, resp.clone()));
        return resp;
      } catch (err) {
        // 诊断: 暴露异常详情，防止 1101 崩溃页且帮助定位根因
    ...
    if (url.pathname === "/") {
      ...
      // SCRT 一次性纳管令牌：不存在则生成并落库
      const { results } = await env.DB.prepare(`SELECT value FROM global_config WHERE key = ?1`).bind(SCRIPT_TOKEN_KEY).all();
      let scrt = (results && results.length > 0) ? JSON.parse(results[0].value) : null;
      if (!scrt) {
        scrt = regenerateScriptToken();
        await env.DB.prepare(`INSERT INTO global_config (key, value) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value = excluded.value`).bind(SCRIPT_TOKEN_KEY, JSON.stringify(scrt)).run();
      }
            ...
            <div class="flex flex-col gap-3 w-full md:w-auto">
                <div class="bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden shadow-lg">
                    <div class="bg-slate-800/50 px-4 py-2 border-b border-slate-700/50 flex items-center gap-2">
                        ...
                        <span class="text-xs text-slate-400 font-mono ml-2">VPS 纳管命令 (Root，含一次性令牌)</span>
                    </div>
